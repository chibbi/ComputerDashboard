...
#
# all functions:
#     arp_cache => dont really know what it is for
#     bandwidth => prints out the max bandwidth per interface
...
#     disk_partitions => name
#     docker_processes => useful for my autodeployment maybe, here NOT working because no docker 
...
#     memory_info => info i do NOT need
#     network_connections => -ntu: command NOT found
#     number_of_cpu_cores => no output??
#     ping => ... No such file or directory (don't have a config file of the git repo i got it from)
#     pm2_stats => pm2: command NOT found
#     ram_intensive_processes => name
#     recent_account_logins => no output (guess because i had no last logins???) NOT needed
#     redis => INFO: command not found (guess because i don't have redis?? xD)
#     scheduled_crons => name
#     swap => gives info about swap (where, how much used and how big)
#     upload_transfer_rate => name
#     user_accounts => all user acoount  (name)
#
ECHO=$(type -P echo)
SED=$(type -P sed)
GREP=$(type -P grep)
